...
@override_settings(
    ...
    DISCOURSE_SSO_RETURN_URL="https://192.168.1.27/",
)
# ----------------------------
# Model Tests
# ----------------------------
class DiscourseModelsTestCase(TestCase):
    ...
    def test_sso_event_log_str(self):
        event = SsoEventLog.objects.create(
            user=self.user,
            event_type="login",
            payload_details="Test payload",
            signature="abcdef",
        )
...
class DiscourseSSOViewsTestCase(TestCase):
    def setUp(self):
        self.client = Client()
        self.user = User.objects.create_user(
            username="sso_user", password="secret", email="sso@example.com"
        )
...
class DiscourseAPITestCase(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username="apiuser", password="secret", email="api@example.com"
        )
